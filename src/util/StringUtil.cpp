...
void StringUtil::ReplaceString(std::string& str_Origin,const std::string& str_Err,const std::string& str_Cor)
{
	std::string::size_type pos=0;
	std::string::size_type len_Err=str_Err.size();
	std::string::size_type len_Cor=str_Cor.size();
	while((pos=str_Origin.find(str_Err,pos))!=std::string::npos)
	{
		str_Origin.replace(pos,len_Err,str_Cor);
		pos+=len_Cor;
	}
}
