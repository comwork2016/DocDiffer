...
/**
    提取两个句子的最长相似子序列
*/
void LongestSimilarSentence::GetSimBoundary(const std::vector<SplitedHits> vec_SearchWords,const std::vector<SplitedHits> vec_OriginWords,std::vector<SenRangeSimilarity>& vec_SenRangeSimilarity)
{
    ...
    for(int k=0; k< vec_SenRangeSimilarityTmp.size(); k++)
    {
        ...
        Range range_SearchNo = senRangeSimilarity.range_SearchNo;
        Range range_SimilarNo = senRangeSimilarity.range_SimilarNo;
        senRangeSimilarity.similarity = CalcSentenceSimilarity(matrix,range_SearchNo,range_SimilarNo);//计算两个范围内句子的相似度
